...
class HttpFetcher {
  ...
  String _getUrl(String url) {
    return repoUrl.replaceAll("\?.*", "").replaceAll("\/\$", "");
  }
  ...
  String _refWantRequst(List<GitRef> wantRefs, List<GitRef> haveRefs, String shallow,
      depth, bool moreHaves) {
    String request = "0067want ${wantRefs[0].sha} "
        + "muti_ack_detailed side-band-64k thin-pack ofs-delta\n";
    for (int i = 1; i < wantRefs.length; ++i) {
      request += "0032want ${wantRefs[i].sha}\n";
    }

    if (haveRefs.isNotEmpty) {
      if (shallow.isNotEmpty) {
        request += "0034shallow " + shallow;
      }
      request += "0000";
      haveRefs.forEach((GitRef ref) {
        request += "0032have ${ref.sha}\n";
      });
      if (moreHaves) {
        request += "0000";
      } else {
      ...
      if (depth) {
        ...
        request += (_padWithZeros(depthStr.length + 4) + depthStr);
      }
      request += "0000";
      request += "0009done\n";
    }
   ...
   */
  Map<String, String> queryParams(String uri) {
  ...
  Future<String> doGet(String url) {
    ...
    urlOptions = queryParams(repoUrl);
  ...
  _addRef(GitRef ref) {
    String type;
    String name;
    if (ref.name.substring(0,5) == "refs/") {
  ...
  Future<List<GitRef>> fetchReceiveRefs() {
    return _fetchRefs('git-receive-pack');
  }

  Future<List<GitRef>> fetchUploadRefs() {
    return _fetchRefs('git-upload-pack');
  }
  ...
  Future fetchRef(List<GitRef> wantRefs, List<GitRef> haveRefs, String shallow,
      int depth, bool moreHaves, noCommon, progress) {
    Completer completer = new Completer();
    ...
    xhr.onLoad.listen((event) {
      ...
      if (haveRefs.isNotEmpty) {
        ...
        //UploadPackParser.parse(binaryData, store, packProgress).then((obj) {
